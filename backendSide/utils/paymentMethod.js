...
const checkoutNodeJssdk = require('@paypal/checkout-server-sdk');
const axios = require('axios');
const logger = require('../startup/logging'); // تأكد من وجود هذا المسار
...
// دالة لجلب Access Token (إذا كنت لا تزال تحتاجها لـ axios.post مباشرة)
async function generateAccessToken() {
    try {
        const auth = Buffer.from(`${process.env.PAYPAL_CLIENT_ID}:${process.env.PAYPAL_CLIENT_SECRET}`).toString("base64");
        const response = await axios.post(
        ...
        logger.error(`Error generating PayPal Access Token: ${ex.response ? ex.response.data : ex.message}`);
        throw ex; // أعد رمي الخطأ
    }
}
